...
const ClientsSection = () => {
  ...
  return (
    <section className="relative py-16 md:py-24 overflow-hidden bg-[#0d0b13]">
      ...
      <div className="relative z-10">
        <div className="container mx-auto px-4 md:px-6 text-center mb-10 md:mb-14">
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium bg-brand-yellow/20 text-white border border-brand-yellow/40">
            Confían en mí
          </span>
          <h2 className="mt-5 text-2xl md:text-3xl lg:text-4xl font-bold text-white">
        ...
        <div className="relative">
          <div className="pointer-events-none absolute inset-y-0 left-0 w-16 md:w-32 z-10 bg-gradient-to-r from-[#0d0b13] to-transparent" />
          <div className="pointer-events-none absolute inset-y-0 right-0 w-16 md:w-32 z-10 bg-gradient-to-l from-[#0d0b13] to-transparent" />

          <div className="flex w-max animate-marquee gap-5 md:gap-7 hover:[animation-play-state:paused]">
            {loop.map((client, i) => (
              <div
                key={i}
                className="group relative flex h-24 w-44 md:h-28 md:w-56 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.08] to-white/[0.03] p-6 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-brand-yellow/60 hover:shadow-[0_0_28px_hsl(var(--brand-yellow)/0.4)]"
              >
